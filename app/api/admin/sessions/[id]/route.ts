...
// Never serialize blob_data into JSON — replay events have their own
// endpoint.
const { blobData: _blobData, ...sessionColumns } = getTableColumns(schema.sessions);
...
export async function GET(_req: NextRequest | Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const rows = await db.select({
    s: sessionColumns, p: getTableColumns(schema.projects),
